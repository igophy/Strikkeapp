...
function resetGarnAlternativPage() {
    if (typeof resetGarnAlternativForm === 'function') {
        resetGarnAlternativForm();
        return;
    }
...
function selectGarnForAlternatives(yarn) {
    ...
    renderSearchResults([], '');
    document.getElementById('garnaltSummary')?.classList.add('hidden');
    document.getElementById('garnaltSummary') && (document.getElementById('garnaltSummary').innerHTML = '');
    document.getElementById('garnaltResultat')?.classList.add('hidden');
    document.getElementById('garnaltResultat') && (document.getElementById('garnaltResultat').innerHTML = '');
}
...
function renderGarnAltResults(source, alternatives, filters, originalSkeins) {
    ...
    if (!alternatives.length) {
        document.getElementById('garnaltSummary')?.classList.add('hidden');
        visResultat(result, '<h3>Fant ingen alternative garn med valgte egenskaper</h3><p>Prøv å fjerne noen valg eller velg et annet originalgarn.</p>');
        return;
    }
...
function resetGarnAlternativForm() {
    const searchInput = document.getElementById('garnSearch');
    const resultBox = document.getElementById('garnaltResultat');
    const summaryBox = document.getElementById('garnaltSummary');
    if (searchInput) {
        searchInput.value = '';
        searchInput.dataset.selectedId = '';
    }
    const skeinsInput = document.getElementById('antallNoster');
    if (skeinsInput) skeinsInput.value = '';
    if (resultBox) {
        resultBox.classList.add('hidden');
        resultBox.innerHTML = '';
    }
...
function setupGarnAlternativ() {
    ...
    findButton.addEventListener('click', () => {
        ...
        const source = getSelectedYarn();
        if (!source) {
            visResultat(resultBox, '<p class="error">Velg et originalgarn først.</p>');
            return;
        }
        showGarnAltResults();
    });
